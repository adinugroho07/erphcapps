              ...
              <div class="col-span-6 sm:col-span-6" >
                ...
                <div id="history" class="shadow overflow-auto border-b border-gray-200 sm:rounded-lg">
                  <table class="min-w-full divide-y divide-gray-200 ">
                    ...
                    <tr v-for="(history,index) in cutihistory" :key="history.id">
        ...
        <form >
          <div class="modal-body relative p-4">
            ...
            <div class="">
              ...
              <jet-input id="applicantcode" type="text" class="mt-1 block w-full" v-model="modalValue.cuticode" disabled/>
            </div>
...
export default defineComponent({
  ...
  props: ['cuti','assignment','assignmentnow','cutihistory'],
  data(){
    return{
      show: false,
      modalValue: this.$inertia.form({
        cuticode: this.cuti.cuticode,
        status: this.cuti.status,
        id: this.cuti.id,
        assignment_id: this.cuti.assignment_id,
        assignment_code: this.cuti.assignment_code,
        memo: '',
        action: ''
      }),
      form: this.$inertia.form({
        ...
        status: 'DRAFT' ,
        cuticode: this.cuti.cuticode ,
        isedit: this.cuti.isedit ,
        department: this.cuti.department ,
        department_code: this.cuti.department_code ,
        posname: this.cuti.posname ,
        poscode: this.cuti.poscode ,
        name: this.cuti.name ,
        email: this.cuti.email ,
        description: this.cuti.description ,
        nik: this.cuti.nik ,
        typecuti: this.cuti.typecuti ,
        startdate: this.cuti.startdate ,
        enddate: this.cuti.enddate ,
        created_by: this.$page.props.user.name ,
        created_byid: this.$page.props.user.id ,
        attachment1: this.cuti.attachment1 ,
        attachment2: this.cuti.attachment2 ,
      }),
      optionsTypeCuti: [
  ...
  methods:{
    ...
    reject(){
      ...
      this.modalValue.post(route('cuti.approve'), {
        onSuccess: () => {
