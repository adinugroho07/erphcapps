  ...
  setup(props) {
    ...
    const deleteUser = (user) => {
      if (confirm("Do you really want to delete user "+user.name+" ?")){
        ...
        form.id = user.id;
        form.post(route('users.deactive'), {
          preserveScroll: true,
          onSuccess: () => {
            notif('Success','Deactivated User '+user.name+' Successfully','success');
          },
          onError: () => {
            notif('Error Occured','There Is An Error When Processing A Non Active User','danger');
          }
        });
      }
    };
